const fs = require('fs')
const path = require('path')

const Cart = require('./cart')

const _path = path.join(process.cwd(), 'data', 'products.json')

const getAllProdsFromFile = cb => {
    fs.readFile(_path, (err, data) => {
        if(err || data.length === 0) {
            return cb([])
        }
        return cb(JSON.parse(data))
    })
}

const wf = body => {
    fs.writeFile(_path, JSON.stringify(body), (err) => {
        if(err) {
            console.log(err);
        }
    })
}

module.exports = class Products {
    constructor({title = "book", price = 0, description = "no description", imageUrl = "data:image/jpeg;base64,"}) {
        this.title = title ? title : 'book'
        this.imageUrl = imageUrl ? imageUrl : "data:image/jpeg;base64,"
        this.price = price ? +price : 0
        this.description = description ? description : 'no desc'
        this.id = Math.random().toString()
    }

    save = () => {
        getAllProdsFromFile(products => {
            products.push(this)
            wf(products)
        })
    }

    static fetchAll = (cb) => {
        getAllProdsFromFile(cb)
    }

    static getProductById = (id, cb) => {
        getAllProdsFromFile(products => {
            const prod = products.find(p => p.id === id)
            cb(prod)
        })
    }


    static editProductById = (id, updatedProduct) => {
        getAllProdsFromFile(products => {
            const idx = products.findIndex(prod => prod.id === id)
            products[idx] = {...updatedProduct, id: id}
            wf(products)
        })
    }

    static deleteProductById = (id) => {
        getAllProdsFromFile(products => {
            const updatedProducts = products.filter(prod => prod.id !== id)
            const product = products.find(prod => prod.id === id)
            const prodPrice = product.price
            Cart.deleteById(id, +prodPrice)
            wf(updatedProducts)
        })
    }
}